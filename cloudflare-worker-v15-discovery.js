...
- Keep normal answers easy to read on a phone.
...
- Treat every result as approximate and tell the seller what to verify.
  ...
  if (!measurements.visible_reference) {
    return reply({ error: "Tro could not see a readable measuring tape. Place the tape on the garment with the zero mark and numbers showing, then take the photo straight down." }, 422, origin);
  }
...
Clearly explain when more information or photographs are needed.
  ...
  const { response: openAIResponse, result } = await callOpenAI(env, {
    ...
    input: [
      {
        role: "developer",
        content: [{ type: "input_text", text: "You are Tro, SourceTro's careful resale assistant. Produce accurate, practical listing help. Never claim live sold comparisons unless the seller supplied them." }],
      },
      { role: "user", content },
    ],
    text: {
...
async function discoverWeb(body, env, origin) {
  ...
  let identification = "{}";
  try { identification = JSON.stringify(body.identification && typeof body.identification === "object" ? body.identification : {}).slice(0, 3500); } catch {}
  if (!image && !query) return reply({ error: "A photo or search phrase is required for discovery." }, 400, origin);

  const userContent = [{
    type: "input_text",
    text: `
Find current public web pages that appear to offer this photographed resale item or a close comparable for sale.

Current search phrase: ${query || "Use the photograph to identify the item first."}
Photo identification hints: ${identification}
Seller country: ${String(body.sellerCountry || "US").slice(0, 20)}

Search broadly across accessible public marketplaces, retailers, resale shops, vintage/antique stores, brand sites, and specialty stores when relevant.
Prioritize pages that show a CURRENT visible price in USD.
Do not claim a listing is sold/completed unless the source explicitly says so.
Do not invent a price, URL, brand, model, exact-match claim, or availability.
For every match, classify it as exact, likely, or similar. Exact means distinctive visible details strongly line up; likely means a strong but not complete match; similar means useful pricing context only.
Omit pages without a usable price. Omit duplicate pages. Return at most 12 matches.
The final summary should be short and useful to a reseller.
    `.trim(),
  }];
    ...
    input: [
      {
        ...
        content: [{
          type: "input_text",
          text: "You are SourceTro Discovery, a resale product-research assistant. Use web search to find real current pages and prices. Be conservative about identity. Never fabricate a price or product URL, and never convert asking prices into sold-price claims.",
        }],
      },
            ...
            matches: {
              ...
              items: {
                ...
                properties: {
                  ...
                  match_type: { type: "string", enum: ["exact", "likely", "similar"] },
                  price_type: { type: "string", enum: ["active_asking", "retail", "unknown"] },
                  confidence: { type: "string", enum: ["high", "medium", "low"] },
                  why_match: { type: "string" },
                },
                required: ["source", "title", "url", "price", "currency", "match_type", "price_type", "confidence", "why_match"],
              },
            },
          },
  ...
  if (!openAIResponse.ok) {
    console.error("OpenAI web discovery request failed:", result);
    return reply({ error: "Tro could not complete broad web discovery right now." }, 502, origin);
  }
  ...
  const matches = (Array.isArray(parsed.matches) ? parsed.matches : [])
    ...
    .map((item) => ({
      ...
      currency: "USD",
      source_verified: sourceHosts.size ? sourceHosts.has(hostOf(item.url)) : false,
    }));
  ...
  return reply({
    ...
    priceMeaning: "current asking/retail prices unless the source explicitly states otherwise",
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET") {
      return reply({
        ...
        smartDiscoveryReady: Boolean(env.OPENAI_API_KEY && env.SOURCETRO_OWNER_KEY),
        transport: "secure-body-v17-cloud-sync-live-chat-smart-discovery",
        message: "SourceTro secure AI, live Tro chat, and Smart Discovery are ready.",
      }, 200, origin);
    }

    if (request.method !== "POST" || !["/analyze", "/measure", "/chat", "/discover-web"].includes(url.pathname)) {
